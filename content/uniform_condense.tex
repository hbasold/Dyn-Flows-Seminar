...
% BjH: Ab hier wird aus ``zeiterweitert'' ``zeit-expandiert''.
...
und der Verwendung von \algRef{qtp_core} als Test kann ein Intervall mit
% BjH: Satzbau
    ...
    \begin{proof}
        ...
        \begin{align*}
            n T'/\Delta & = n \lceil (1 + \eps)^3 T/\Delta \rceil \\
                        ...
                        % BjH: Diese Zeile scheint nicht zu stimmen (setze mal 1,1 als zu rundenden Wert oben ein). Wenn du sie streichst, stimmt die folgende Ungleichung aber wieder.
        ...
        % BjH: Wenn du die Herleitung f\"ur die Anzahl der Kanten machst und dann sagst, bei den Knoten muss $m$ durch $n$ ersetzt werden, wird deutlicher, warum zumindest ein $n$ in beiden Ausdr\"ucken steht.
...
\begin{theorem}\label{theo:slow_flow}
    ...
    \]
    % BjH: Hier w\"urde ich erg\"anzen, dass $\bar{f}$ gem\"a\ss{} dem letzten Lemma definiert sein soll, da alle anderen Gr\"o\ss{}en auch nochmal innerhalb dieses Satzes definiert wurden.
...
% BjH: Worauf bezieht sich ``Dies'' genau?
...
% BjH: Wo war diese Definition? Ohne zumindest eine Seitenzahl ist die Referenz f\"ur Leser kaum brauchbar.
