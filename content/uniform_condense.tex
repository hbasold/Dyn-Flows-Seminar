...
Allerdings kann einiges von den Beweisen wieder verwendet werden.
...
\label{prob:qtp_single}
    Sei ein Netzwerk $(\graph, \tau)$ gegeben. Seien weiterhin $D > 0$ und $C > 0$
    gegeben. Gesucht ist ein Fluss $\func{f^*}{\ropen{0,T^*}}{\R_+}$ in $\graph$,
    so dass $|f| = D$ und $c(f) \leq C$ und $T^*$ minimal für alle Flüsse, die
...
\begin{theorem}
    ...
    für \probRef{qtp_single} bestimmt werden.
    \begin{proof}
        TODO: kurze Diskussion.
    \end{proof}
\end{theorem}
